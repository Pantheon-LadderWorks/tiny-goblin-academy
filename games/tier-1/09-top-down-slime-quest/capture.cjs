...
async function run() {
  const captureRun = prepareCaptureRun({
    ...
    gameId: 'level-09-top-down-slime-quest',
  ...
  await page.screenshot({ path: captureRun.file('stills', '02-pounce.png') });
  ...
  await page.screenshot({ path: captureRun.file('stills', '03-defeat.png') });
  ...
  await page.screenshot({ path: captureRun.file('stills', '04-victory.png') });

  await browser.close();
  finalizeCaptureRun(captureRun, {
    captureScript: 'games/tier-1/09-top-down-slime-quest/capture.cjs',
