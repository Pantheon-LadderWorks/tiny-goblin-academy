...
import { finalizeCaptureRun, prepareCaptureRun } from '../../../../tools/evidence/capture-run.mjs';
...
async function run() {
  ...
  await page.screenshot({ path: captureRun.file('stills', '01-initial-state.png') });
  ...
  await page.screenshot({ path: captureRun.file('stills', '02-hit-reaction.png') });
  ...
  await page.screenshot({ path: captureRun.file('stills', '03-coins-earned.png') });

  console.log('Closing browser and server...');
  await browser.close();
  await server.close();
  finalizeCaptureRun(captureRun, {
    captureScript: 'games/tier-1/01-button-goblin-clicker/scripts/capture-evidence.mjs',
