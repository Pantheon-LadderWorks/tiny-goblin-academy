...
def analyze_grid(path):
    ...
    if len(grid_cols) < 2 or len(grid_rows) < 2:
        print("Failed to find explicit grid lines.")
        # fallback to previous logic
        h, w, _ = arr.shape
        grid_cols = np.linspace(0, w, 17).astype(int)
        grid_rows = np.linspace(0, h, 9).astype(int)
        print(f"Fallback to 16x8 matrix based on dimension divisions.")
...
def main():
    ...
    analyze_grid('assets/academy/creatures/soldier/tga-topdown-soldier-enemy-v0.1.png')
