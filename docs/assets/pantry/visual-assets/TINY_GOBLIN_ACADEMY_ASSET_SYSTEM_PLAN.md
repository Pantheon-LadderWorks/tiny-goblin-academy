...
## H5.69 Topdown Objects Non-FX Cleanup Human Review Note

H5.69 accepts the H5.68 Topdown Objects selective non-FX cleanup candidate for draft cleanup/planning use with one product exclusion.

Accepted usable non-FX regions:

```text
51
```

Region 1 is excluded by product choice / not-selected-for-use. It is not marked as cleanup-failed.

The H5.68 effect/glow/fire/portal/smoke/slime/shadow exclusions remain excluded:
...
## H5.102A Runtime Material and Containment Preparation Correction Note

H5.102 resolves eighteen direct local material sources and frames all thirty-two H5.48C regions from the cleaned regenerated atlas without creating duplicate sprite files. The two H5.49 denials remain non-selectable reference records. The preparation contracts separate PotionActorRig presentation ownership from holder-local containment and keep visible, mask, interaction, and sorting/drop bounds independent.

Containment follows a minimum-complexity hierarchy: depth layering first, holder-local geometry masks only for deep receivers or apertures, and authored alpha masks only after an actual irregular-opening gap is demonstrated. H5.102A replaces the rejected four-slot-bin proof with single-color receiving rigs using regions 17–19, separates finished presentation from diagnostic contours, proves approach/partial/accepted containment and approach/partial/exit aperture passage, and supplies readable inventory evidence at both desktop contracts. The H5.48C red sorter slot still does not demonstrate an alpha-mask gap; painted identity art plus simple invisible geometry is sufficient. The Phaser proof harness, generated manifests, inventories, contracts, interaction/layout proofs, and nineteen captures are evidence-only. Human re-review remains required and no runtime binding, room or conveyor SceneRig, gameplay change, emitter, shader, package change, or runtime approval is authorized.
