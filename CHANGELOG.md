...
* **H6.4A Button Goblin Material Typography and Desktop Contract**: Added the smallest reusable shared host-surface primitive and selectively integrated real Academy UI/HUD physical assets into Button Goblin Clicker. Region 20 hosts the victory title/body/footer; Region 30 hosts only the `ONE UPGRADE` label; Region 5 remains rejected because four uniform HUD instances crowd the actor/name lanes and one cannot host all four values. Material-aware recipes now use Cinzel on the result title, Caudex on parchment body copy, and Outfit on paper labels; the Region 30 host is 160px wide, the masthead/HUD hierarchy is quieter, and the victory plaque layers above the HUD at minimum size. Actual Tauri evidence covers all seven states at the configured `1024×640` minimum content contract plus clean initial/victory anchors on the `1920×1080` primary display. Evidence now lives with the game under `games/tier-1/01-button-goblin-clicker/evidence/`. Human visual review passed and the packet closed in `ef903b1`; H6.7A later strengthened its shared title/result hierarchy in `20c0b0f`. No gameplay, controller/simulation, GoblinRig, hit area, animation, background, Academy shell, Rust/Tauri, package/lock, source-sheet, remap, or other game behavior changed.

All notable changes to this project will be documented in this file.

## Unreleased

* **H6.17B GitHub LFS Blocker and Evidence Policy Pivot**: Recorded that GitHub rejected H6.17's first and only 792-byte synthetic LFS object because LFS access is billing-blocked. No synthetic payload was accepted, no real evidence payload was uploaded, no temporary Git ref was created, and canonical C remained unchanged. Preserved H6.15-H6.17 as truthful historical engineering records while superseding the otherwise validated LFS/dual-checkout architecture. Existing evidence is grandfathered in ordinary Git; future heavy generated review media defaults to durable external D storage represented by lightweight tracked manifests, hashes, sizes, provenance, review state, scripts, and selected compact proof images under a provisional 2 MiB ceiling. No additional LFS attempt, dual-checkout implementation, history rewrite, evidence move, runtime/game change, package/lock change, or Card Goblin Duel work is authorized. Emergency ordinary-Git backlog backup is the immediate preservation lane and is pending at this commit.

* **Dice Duel Tavern Curriculum and Visual Integration Closure**: Closed Level 03’s mechanical and Tier 1.5 visual learning records without rewriting the original fixed-sequence First Playable as the finished runtime. The completed chapter records H5.30–H5.33 asset preparation, H6.9 stage-first Crooked Six composition, H6.10 persistent six-face Mesh2D laboratory, H6.11 production Web Crypto d6 and explicit rolling authority, and H6.12 materials, selective UI assets, typography, and terminal plaques. Corrected stale playtest, release, roster, progress, roadmap, README, and evidence-index authority; recorded build capability, human runtime/visual approval, and repository/developer-mode Hub behavior while preserving no standalone game executable and no production-distribution-mode claim. No runtime source, evidence binary, Hub/Tauri behavior, package/lockfile, or other game changed. Evidence-size inventory and D-drive warehouse migration remain the immediate next lane before Card Goblin Duel.

* **Hub External-Build Workspace Resolution**: Corrected the local developer Hub workspace resolver so release executables built outside the repository search the current working directory, executable ancestors, then the manifest-validated source workspace captured at build time. Human runtime proof passed from both a desktop shortcut and direct File Explorer launch of `D:\DevCache\Rust\targets\tiny-goblin-academy\release\app.exe` without a repository-root working directory; both routes found the C-drive Academy workspace and hydrated its games. This fallback does not bundle games, hardcode individual game paths, or create a portable production install; moving the repository requires rebuilding the local Hub.

* **H6.7/H6.7A Potion Sorter Shared Typography, Result Surface, and Cross-Game Display Hierarchy**: Applied the repository-local Academy typography runtime across Potion Sorter's DOM and Phaser layers, kept Time/Score/Combo code-native, and used shared Region 20 only for truthful success and timer-expiry results. Human review returned the first display hierarchy because the improved family remained too delicate; the shared `game-title` and `result-state` recipes now use Cinzel 800 with crisp material-aware shadows, and conflicting local overrides were removed from both Potion Sorter and Button Goblin. Fresh `1920x1080` and `1024x640` runtime evidence proves both games, both result plaques, local font loading, supported-window fit, and unchanged plaque geometry. No simulation/controller, scoring, package/lock, source font, source image, or unrelated game behavior changed.

* **H6.5-H6.6 Potion Sorter Stage-First Shell and Live Composition C SceneRig**: Migrated Potion Sorter out of the permanent lesson-dashboard rails, then integrated the human-approved Composition C medieval alchemy room as individually addressable environment, machine, conveyor, potion, and receiver rigs. Preserved tap-select and added aperture drag as parallel presentation inputs into the same controller path. Corrected the first preview's lost perspective, duplicate bottle actors, render-ownership gaps, black inspection void, incorrect frame depth, receiver seating, and a cluttered right-wing bottle rack; the final asymmetric room uses botanicals left, sorting authority center, and a sparse gearbox/service bay right. Runtime, gameplay, minimum-window, process, and human visual review passed without changing simulation values or matching rules.

* **H5.102A Potion Sorter Runtime Material and Containment Evidence Correction**: Preserved the provisionally accepted H5.102 architecture while rebuilding its rejected first visual-proof pass. Replaced the unreadable six-column inventory with a four-column primary plate and two 1024×640 minimum-window plates backed by live overflow/collision checks. Removed region 23's four-slot organizer from the primary deep-containment proof and used regions 17–19 as actual red/blue/green receiving identities with matching potions, consistent anchors, invisible holder-local masks, and lower foreground lips. Presentation and debug geometry are now separated; the aperture proves approach/partial/exit synchronization; cradle and rail seating were tuned. The packet now contains nineteen current captures and removes superseded first-pass PNGs. Human review passed the corrected evidence and approved runtime preparation only, making the preview-only H6 SceneRig lane ready. No Potion Sorter runtime, SceneRig, gameplay, simulation/controller, source image, package/lock, emitter, shader, or runtime registry changed; every material and skin remains not runtime-approved.

* **H5.101 Potion Sorter Neutral Material Specimen Audition**: Built a reproducible evidence-only Canvas laboratory that places the accepted H5.100/H5.100C sources on eight shared code-authored specimen families under neutral and warm mechanism light. The provisional palette uses stylized fantasy timber, masonry, painted iron, illustrated parchment, authored wear, and code-authored glass as primary identity, with realistic textures constrained to quiet grain, small hardware, wear support, and focal response. Metal008 brass successfully adds contrast and material pop on a limited gear hub or valve rim, while broad realistic brass is explicitly rejected. Added 18 complete provisional recipes, a 25-source resolved inventory, fourteen evidence sheets including 1920×1080 and 1024×640 coherence boards, and reproducible generator/capture/validation tooling. Human review passed and approved the selections for H5.102 runtime preparation only. No Potion Sorter runtime, room, SceneRig, gameplay, particle emitter, shader, package/lock, source image, or runtime registry changed; every recipe remains `runtimeApproved: false`.

* **H5.100C Potion Sorter Stylized Fantasy Texture Pantry Addendum**: Added a bounded fantasy-facing identity layer beside the accepted H5.100 neutral pantry. Preserved Kenney Retro Textures Fantasy 1.0, DeadKir's three hand-painted tileable CC0 images, and Luke.RUSTLTD's large CC0 parchment original with hashes, licenses, metadata, selected texture extraction, material-role classification, provenance evidence, and a realistic-versus-stylized-versus-hybrid comparison. Kenney supplies primary timber/masonry language; DeadKir supplies painted dark metal, a timber alternate, and alchemical ooze; the parchment source fills the illustrated-paper gap. Human review approved these provenance-clean families for the reusable pantry and approved H5.100 Metal008 only as a restrained realistic brass focal accent for warm contrast and material pop. Quaternius model-first packs, Kenney Modular Dungeon palette atlases, and an overlapping hand-painted audition pack were not ingested. Research previews remain evidence-only. No Potion Sorter runtime, room, SceneRig, shader, material binding, package/lock, or gameplay file changed; H5.101 neutral-specimen audition remains required.

* **H5.100 Potion Sorter Texture Material Intake and Provenance**: Established the first bounded GlyphForge material-source pantry for a future code-authored Potion Sorter alchemy production room. Preserved 10 unchanged CC0 source archives from ambientCG and Kenney, extracted only nine 1K color maps plus eight transparent FX audition helpers, recorded SHA-256 provenance and technical classifications, and produced source-family, palette, decision, license, and future SceneRig-mapping evidence. Human review approved all provenance-clean accepted source families and useful alternatives for the reusable pantry while keeping research previews evidence-only. The provisional H5.101 shortlist is WoodSiding008, Bricks089, Metal046B, Metal008, Paper006, SurfaceImperfections015, smoke_06, dirt_02, spark_01, and light_01; Metal053C remains the rust-heavy iron alternate. No external glass or liquid texture was promoted. No Potion Sorter runtime, SceneRig, shader/PBR, particle system, flattened room background, Hub/Tauri, package/lock, runtime registry, or gameplay file changed. H5.101 neutral-specimen audition remains required before any Potion Sorter runtime selection; nothing staged or committed.

* **H5.99 Academy Typography Runtime Integration Proof**: Implemented the H5.98 canonical typography defaults in exactly the Academy Hub and Button Goblin Clicker through one shared repository-local runtime layer. Added local `@font-face` registration, semantic DOM roles, explicit fallbacks, a Phaser TextStyle factory, and font readiness before Phaser text creation. Captured live boot/desktop/Tauri-minimum-window/Help/Ledger/Dev/production-gating Hub evidence plus Button initial/Bonk/upgrade/stronger-goblin/victory/narrow/busy-cavern evidence. Computed inspection recorded 213 DOM samples, zero unloaded faces, zero clipping findings, zero minimum-window horizontal overflow, all eight Phaser readiness descriptors loaded, three actual Phaser recipe/style records, and zero external font/asset requests. Caudex 700 remained readable, so no alternate recipe was promoted. Human visual review passed for the Hub, Button Goblin, and DOM/CSS-to-Phaser parity evidence. No package/lock, gameplay, controller/simulation, GoblinRig, shared UI image, other game, or decorative panel integration changed.

* **H5.98 Academy Typography Promotion and Default Recipe Registry**: Promoted an active Academy font inventory and canonical 15-recipe typography registry from the approved H5.97 laboratory. Designated one initial default for each of 11 semantic roles, retained four material/length variants and the licensed accent pantry, imported current official Google Fonts sources for Outfit, Atkinson Hyperlegible, and Fira Code, and resolved Macondo's inherited license-header encoding damage using the official Google Fonts `METADATA.pb` identity while preserving the SIL OFL 1.1 body. Added concise provenance/default/alternate/contract audit tables. No Hub CSS, Phaser preload/TextStyle, game runtime, package, or lockfile changed; implementation is deferred to H5.99.

* **H5.97 Academy Visual Grammar Laboratory**: Created a static visual laboratory for Academy UI and typography across Hub, Button Goblin Clicker, Potion Sorter, Card Goblin Duel, and One-Room Platformer, with planning coverage for the remaining Tier 1 games. Added controlled fantasy font source intake from the Google Fonts GitHub repository under `assets/academy/fonts/source/google-fonts/`, including OFL/license metadata and SHA-256 provenance records, for evaluation only. H5.97C regenerated shared-host evidence from H5.95 slot geometry; human review passed after Region 22's title lane was optically corrected to `x 0.17 / y 0.14 / w 0.55 / h 0.15`. H5.97D added and visually approved 15 complete selectable `semantic role × host material` recipes plus material, state, real-cavern stress, desktop/narrow, contract-table, and DOM/CSS-to-Phaser parity evidence. The inherited Macondo license-text encoding damage is recorded as H5.98 provenance debt. No runtime source, live CSS, Phaser TextStyle/preload, package/lock files, Button Goblin UI asset selection, font runtime wiring, component runtime approval, or nine-slice approval changed.

* **H5.96 Academy UI Grammar and Font Architecture Plan**: Created an Academy-wide UI ownership and typography plan before Button Goblin Clicker selects any shared UI/HUD assets. Audited the Hub plus all ten Tier 1 games, classified recurring UI responsibilities as academy-shared, shared-skinnable, or game-specific, and defined a semantic font grammar covering Academy display, game display, UI body, labels, data, debug mono, and optional game accents. Recorded that current typography is mostly system/fallback stacks, while H5.89/H5.90 font directions remain planned but not ingested. Created the planning manifest and four table-style evidence previews under `assets/academy/evidence/`. No Button Goblin UI asset selection, runtime UI approval, font ingest, CSS/Phaser edits, package/lock changes, source image changes, or game/runtime wiring occurred.

* **H5.94 Shared UI/HUD Internal Functional Slot Mapping**: Corrected the H5.94 lane boundary to shared UI/HUD internal functional mapping only. Classified all 34 H5.2/H5.2B shared UI/HUD regions by functional role, mapped 11 dynamic host surfaces with surface-relative draft slot rectangles, safe insets, protected decoration zones, and fit/overflow notes, and created corrected shared-only evidence under `assets/academy/evidence/`. Button Goblin UI asset selection is deferred to a later planning lane. No Button Goblin runtime files, game-specific UI selection manifest, source/derived UI images, package/lock files, shell overlay behavior, or runtime UI approval changed.

* **H5.95 Shared UI/HUD Functional Slot Human Review**: Recorded human/product review pass for the H5.94/H5.94A shared UI/HUD internal functional slot mapping. Accepted all 34 region classifications, 11 dynamic host-surface identifications, 18 surface-relative draft slots, protected border/decorative exclusions, and the corrected evidence legibility. Promoted `manifests/academy/shared/planning/academy.ui-hud.functional-surfaces.json` to reviewed draft planning use while keeping `runtimeEligibility: not-runtime-approved`, `gameSelectionApproval: none`, and `nineSliceApproval: none`. Button Goblin shared UI selection remains deferred to H5.96; no runtime/game files, source/derived UI images, package/lock files, or game-specific selection manifests changed.

* **H6.3C Button Goblin Live GoblinRig Integration**: Replaced the live Button Goblin Clicker floating head renderer with the H6.3B-approved full-body `GoblinRig` while preserving controller/simulation ownership and gameplay values. The live actor now uses the central cavern safe-zone placement, a full-body 250x305 ellipse hit area, hover/Bonk -1/Bonk -2/defeat/reset/victory presentation wiring, and lifecycle cleanup guards for tweens/timers after teardown. Captured browser and embedded Tauri evidence for desktop, narrow, debug-hit-area, upgrade, stronger-goblin, reset, victory, outside-click rejection, and Close Game cleanup states. Human review passed for idle breathing, blinking, ear twitching, hover scale, click/bonk jolt, full-body hit area, defeat/reset, upgrade progression, victory, and process cleanup. No package/lock, controller, simulation, Rust/Tauri, background source image, or other game changes occurred.

* **H6.3B Button Goblin Vector Actor Rig Preview**: Added a preview-only Phaser vector actor rig for Button Goblin Clicker so the current simple goblin head identity can be tested as a reusable full-body code-authored puppet before replacing the live runtime actor. The rig preserves the round green head, triangular ears, cream eyes, fang, and simple Academy style while adding a torso, ragged tunic, belt, arms, legs, feet, shadow, hover, bonk, defeat, reset, and variant preview states. Created a standalone preview page and evidence using the approved H6.3 cavern background with anchor overlays, actor bounds, hit area, and foot baseline. The live Button Goblin runtime still uses the existing H6.2/H6.3 goblin head; no gameplay/controller/simulation changes, sprite sheets, custom art skins, exact runtime coordinates, Tauri/Rust changes, or other game migrations occurred.

* **H6.3 Button Goblin Clicker Background Stage Integration**: Wired the H5.85/H5.86 Button Goblin Clicker cavern background into the live Phaser play surface as the first narrow Tier 1.5 runtime visual asset integration. The background now loads through Vite, renders behind the existing SVG/Phaser goblin click target with cover-style scaling and a conservative readability scrim, and falls back to the Academy slate stage if the texture is unavailable. Updated the active background manifest to record runtime approval for Button Goblin background-stage use only; exact coordinates, UI sprite sheets, goblin sprite replacement, other games, gameplay behavior, and global visual pantry approval remain unapproved. Direct browser evidence was captured for desktop initial/hit/upgrade/victory and narrow initial/hit states. Human/product review accepted the background anchor map as a design contract and identified H6.3B as the future code-authored vector goblin actor preview lane. No simulation/controller rules, hub shell architecture, Rust/Tauri lifecycle code, package/lock files, or other games changed.

* **H6.2 Button Goblin Clicker Shell Migration**: Migrated Button Goblin Clicker from the old permanent side-panel dashboard layout into the first stage-first game composition. The left statistics rail is now an in-stage HUD layer, the central Phaser goblin target remains the dominant play surface, and the old right upgrade rail is now an in-stage Bonk Stick action card. H6.2A moved the transient `-1 BONK!` feedback into a separate upper-right feedback lane so it no longer collides with the stable `Goblin #1` encounter label, and added final review evidence for desktop/narrow hit states, defeat/coin reward, upgrade availability/purchase, later increased HP, and victory. Human visual/Tauri review passed for stage-first composition, desktop/narrow layouts, corrected feedback, gameplay loop preservation, and Help/Ledger/Dev shell overlays. Preserved the existing simulation/controller rules, goblin click behavior, Bonk Stick upgrade, victory loop, and placeholder Phaser goblin; no background asset wiring, shared HUD abstraction, Ledger hydration, Help/Dev data bridge, stateful Academy progression, package/lock changes, Tauri/Rust/dev-server changes, or other game migrations occurred.

* **H6.1 Minimal Runtime Shell Contract + Stale Manifest Path Cleanup**: Added the first runtime shell contract layer in `DevGameRuntimeView` with compact top-bar controls for Ledger, Help, Dev, and Close. Ledger now has a shell-level button plus `L` shortcut, while Help and Dev have draft overlay surfaces for future per-game migration. Fixed active stale manifest imports in the hub roster, hub icon data, One-Room Platformer, and the Tauri Rust backend so they point at the nested H5.91/H5.91A manifest shelves. The backend now uses `ACADEMY_GAMES_MANIFEST_PATH` for workspace discovery and manifest loading. H6.1B also adds Academy-owned dev-server lifecycle cleanup: Windows game launches are assigned to an Academy-owned Job Object with kill-on-close, Close Game and stop-all cleanup share one backend path, and red-X/app-exit events now stop tracked game dev-server process trees before exit. The observed One-Room Platformer orphan on port 5108 was terminated by exact TGA-owned PIDs only, root-only `pnpm install --ignore-scripts` then completed, and `cargo check` passed from `hub/src-tauri`. H6.1D keeps the TGA Rust target tree on `D:/DevCache/Rust/targets/tiny-goblin-academy` while disabling project-local Cargo incremental compilation so exFAT is no longer asked to provide unsupported hard-link behavior. Captured the future Ledger hydration contract plus the recovered stateful Academy generator/sink progression doctrine as planning notes only. Created the H6.1 runtime report and planning manifest. Human Tauri review passed, including red-X cleanup and port `5108` release. No Button Goblin migration, visual asset wiring, per-game side-panel removal, Top-Down Slime Quest runtime work, package/lock changes, or PNG/image changes occurred.

* **H6.0 Academy Shell Refactor and In-Game HUD Plan**: Planned the Tier 1.5 runtime visual integration pivot without changing runtime code. Documented the current two-layer shell problem, proposed a minimal Academy top bar, moved the Action Ledger behind a button plus `L` shortcut, routed Help and Dev state into modals/overlays, and defined in-stage HUD/control rules so side panels can be removed without losing player-facing controls. Added an explicit UI sourcing policy: use asset-backed UI for reviewed frames/icons/backplates, code-native DOM/CSS for dynamic text, modals, responsive layout, and accessibility, hybrid UI for asset frames plus live data, and particle/code-generated FX for glow/fire/smoke. Recorded Button Goblin Clicker as the first runtime visual candidate and captured stale manifest import risks for the hub roster and One-Room Platformer as H6 implementation prerequisites. No runtime/game code, PNGs/images, package/lock files, docs/assets reorganization, or manifest reorganization changed.

* **H5.93 Active/Stale Reference Update Pass**: Corrected current-facing overview and discovery docs after H5.91/H5.91A/H5.92. Updated active Level 1 / Button Goblin Clicker language to reflect restored source availability, corrected nested manifest and docs/assets shelf references, and recorded that runtime visual integration remains future work with Button Goblin Clicker first and Top-Down Slime Quest last or near-last. GlyphForge remains paused at H5.88F and font binary intake remains future work. Historical reports and incident records were intentionally preserved as fossil record; no files were moved, no folders reorganized, and no runtime/game code, PNGs/images, package/lock files, or dependency installs changed.

* **H5.92 Asset Docs Reorganization Move Pass**: Re-surveyed the current `docs/assets/` shelf instead of relying on stale H5.87 counts, then moved `128` tracked loose asset docs into categorized shelves using `git mv`. Added `docs/assets/README.md`, created `docs/assets/tooling/organization/TINY_GOBLIN_ACADEMY_H5_92_ASSET_DOCS_REORGANIZATION_MOVE_PASS.md`, and recorded the execution at `manifests/academy/tooling/organization/execution/academy.asset-docs-reorganization-execution.json`. The active asset system plan now lives at `docs/assets/pantry/visual-assets/TINY_GOBLIN_ACADEMY_ASSET_SYSTEM_PLAN.md`; historical lane reports are nested by doctrine, hub visuals, shared assets, game assets, topdown lanes, pipeline, pantry, and tooling. No docs were deleted, no evidence images were moved, no manifests were reorganized, and no runtime/game code, PNGs/images, package/lock files, or dependency installs changed.

* **H5.91A Manifest Authority Subfolder Refinement**: Refined the H5.91 manifest shelves by maturity/authority while keeping active/current manifests visible at each category root. Moved `29` non-current manifests with `git mv` into `lineage/`, `planning/`, `execution/`, and `deferred/` subfolders, created `manifests/academy/tooling/organization/academy.manifest-maturity-index.json`, and documented active counts by category. Pet Campfire now shows 3 active/current manifests at first glance instead of 9 equal-looking mixed-authority files. Updated active manifest references, validators/scripts, the GlyphForge registry paths, and `manifests/README.md`. No manifests were deleted, no `docs/assets` folder reorganization occurred, no runtime/game code, PNGs/images, package/lock files, or dependency installs changed.

* **H5.91 Manifest Folder Reorganization Move Pass**: Re-surveyed the current `manifests/` shelf instead of relying on stale H5.87 counts, then moved all `61` tracked root JSON manifests into categorized folders using `git mv`. Added `manifests/README.md` shelf guidance and `manifests/academy/tooling/organization/academy.manifest-folder-reorganization-execution.json` as the execution record. Updated active manifest references, validators, asset-pipeline docs/scripts, and the GlyphForge registry generator so nested paths are the active source of truth. Regenerated the GlyphForge visual registry with 24 valid entries. Root `manifests/*.json` is now `0`, no destination folder has more than 10 loose JSON files, and no runtime/game code, PNGs/images, package/lock files, dependency installs, docs/assets reorganization, or stale overview-doc correction occurred.

* **H5.90 Font Pantry Human Review and New Fantasy Core Selection**: Reviewed the H5.89 font pantry inventory and redirected Tiny Goblin Academy font identity away from overusing Kryssie Method ebook fonts. Created `manifests/academy.font-pantry-core-selection.json` and selected a new fantasy/storybook core direction: Cinzel Decorative/Cinzel for shared display, Caudex/Alegreya for readable fantasy UI/body, Atkinson Hyperlegible/Outfit as readable accessibility fallback, and Fira Code/Space Mono for debug/tooling mono. Recorded game flavor candidates including Macondo, Almendra, Cormorant Garamond, Merienda, MedievalSharp, Uncial Antiqua, Metamorphous, Fondamento, Grenze Gotisch, and Eagle Lake for future source/license intake. Ubuntu is not selected for now, Love Light remains decorative-only if ever used, and `ROGLyonsTypeRegular3.ttf` remains do-not-ingest without license proof. No font binaries were copied, no remote fonts fetched, no runtime font wiring occurred, no game code, PNGs/images, package/lock files, dependency installs, tool implementation files, or docs/manifests folder reorganization changed.

* **H5.89 Font Pantry Source Inventory and Intake Plan**: Paused the GlyphForge tool lane after H5.88E and recorded H5.88F as the next tool-lane return point. Created `manifests/academy.font-pantry-source-inventory-plan.json` and documented a Tier 1.5 font pantry intake plan for shared academy identity, readable UI text, debug/tooling text, game-specific flavor fonts, and future local/offline packaging. Inspected local font candidates under the Kryssie Method font root; Atkinson Hyperlegible, Fira Code, Iceberg, Iceland, Love Light, Orbitron, Outfit, Rajdhani, Space Grotesk, and Space Mono have local OFL evidence; Ubuntu has a local UFL license requiring human review; `ROGLyonsTypeRegular3.ttf` has no nearby license and must not be ingested. No font binaries were copied, no runtime font wiring occurred, no game code, PNGs/images, package/lock files, dependency installs, tool implementation files, or docs/manifests folder reorganization changed.

* **H5.88E GlyphForge Static Viewer Shell Human Review**: Recorded human/product review pass with polish notes for the H5.88D static/offline GlyphForge Visual Workbench prototype. Accepted the shell as a local visual review/planning prototype with 24 registry entries, working asset listing, filters, dashboard summary, visible runtime boundary strip, metadata/path/warnings panels, raw registry entry preview, and first useful Region / Asset Browser summary mode. Noted that mode tabs can display non-native selected entries in placeholder views; future maturity should add auto-routing or mismatch warnings. No app behavior files, runtime/game code, PNGs/images, package/lock files, dependency installs, registry entry runtime approvals, or docs/manifests folder reorganization changed.
* **H5.88D GlyphForge Static Viewer Shell Prototype**: Built the first static/offline GlyphForge Visual Workbench prototype under `tools/glyphforge/viewer-shell-static/` with plain HTML/CSS/JS and no build step. Added a built-in-Node registry generator and generated `tools/glyphforge/registry/glyphforge-visual-registry.v0.1.json` from the H5.85 visual asset pantry index. The registry contains 24 entries and routes assets to Region / Asset Browser, Scene Composition, Flipbook, and Particle FX modes. The shell supports registry loading, file-picker fallback, entry listing, text/filter controls, status/runtime boundary strip, path display, warnings/exclusions, raw JSON preview, and a first useful Region / Asset Browser summary mode; other modes remain placeholders. No runtime wiring, game code change, package/lock change, dependency install, image processing, PNG/source/derived/evidence image mutation, prototype modification, or docs/manifests folder reorganization occurred.

* **H5.88C GlyphForge Viewer Shell Static Prototype Plan**: Planned the first static/offline registry-fed GlyphForge Visual Workbench prototype. Proposed `tools/glyphforge/viewer-shell-static/` plus generated registry path `tools/glyphforge/registry/glyphforge-visual-registry.v0.1.json`, defined UI regions, navigation flow, tool-mode routing, registry entry shape, first build scope, evidence plan, validation plan, and runtime safety boundaries. Recommended Region / Asset Browser / Sticker Picture Book Mode as the first useful implementation mode while keeping Scene Composition distinct. No shell/UI was built, no prototype HTML files were changed, no runtime/game code, PNGs, package/lock files, dependency installs, or manifest/docs reorganization occurred.

* **H5.88B GlyphForge Unified Viewer Shell Design**: Designed the unified GlyphForge Visual Workbench shell around six planned modes: dashboard/launcher, Flipbook Viewer, Region / Asset Browser, Scene Composition / Layout Editor, Particle FX Viewer, and future Tier 2.5 Audio Viewer placeholder. Split the Sticker / Picture Book direction into two distinct jobs: region/asset browsing asks what the asset box is, while scene composition asks where the asset/entity goes in a scene. Recorded registry-first navigation, per-mode data contracts, prototype feature sources, safety boundaries, implementation phases, and a first implementation recommendation favoring a future Region Asset Browser / Sticker Picture Book mode after shell planning. No tools were built, no UI was implemented, no runtime wiring occurred, no game code changed, no prototypes were modified, no PNGs/images were changed, no dependencies or package/lock files changed, and no docs/manifests folders were reorganized.

* **H5.88A2 Level 8 Sticker Book Editor Prototype Addendum**: Added the missed Level 8 Sticker Book Editor Downloads prototype to the visual tool prototype comparison as a preserved reference under `tools/glyphforge/prototypes/`. Classified it as a scene-composition / sticker-placement / layout-export prototype used for One Room Dungeon Platformer / Level 8 layout planning, documented its live JSON layout export shape, and updated the GlyphForge direction so Sticker/Picture Book now has two related future modes: region/asset browser and scene composition/layout editor. No runtime wiring, game code change, image processing, PNG modification, dependency install, package/lock change, existing evidence flipbook edit, or docs/manifests reorganization occurred.

* **H5.88A Visual Tool Prototype Intake and Maturity Comparison**: Inspected repo-local flipbook evidence prototypes plus the user-held `sprite-box-annotator.html` and `particle_fx_viewer (1).html` Downloads prototypes. Preserved the two Downloads prototypes under `tools/glyphforge/prototypes/` as reference-only HTML files with a README, created `manifests/academy.visual-tool-prototype-intake-comparison.json`, and documented comparative maturity findings for Flipbook, Sprite Box Annotator, and Particle FX Viewer. Recorded reusable features, rebuild candidates, retire/archive candidates, and the recommended GlyphForge direction: H5.88B unified viewer shell design followed by a registry-fed Sticker/Picture Book region annotator. No runtime/game wiring, image processing, PNG edits, evidence flipbook edits/moves, package/lock changes, dependency installs, or docs/manifests reorganization occurred.

* **H5.88 Visual Tool Suite Registry Plan**: Created a draft planning registry for the future GlyphForge Visual Tool Suite. Defined planned tool surfaces for Flipbook Viewer, Sticker / Picture Book Viewer, Particle FX Viewer, and future Tier 2.5 Audio / Sound Pipeline Viewer. Recorded the pantry-index-first discovery model, viewer data contracts, safety boundaries, implementation phases, and open questions. Inspected existing repo flipbook evidence HTML and two user-held Downloads prototypes (`sprite-box-annotator.html` and `particle_fx_viewer (1).html`) as references only; neither tool was copied, moved, ingested, or made canonical. No tools were built, no runtime wiring occurred, no game code changed, no PNGs or images were modified, no manifests/docs were reorganized, and no package/lock files changed.

* **H5.87 Manifest and Asset Docs Organization Plan**: Created a draft organization plan for the now-large Tiny Goblin Academy manifest and asset-doc shelves. Inventoried the current flat state (`55` root manifest files and `116` root `docs/assets` files), proposed categorized manifest shelves for academy core, hub, shared, creatures, games, topdown lanes, visual pantry, tooling, boot, and legacy records, and proposed `docs/assets` shelves for doctrine, pipeline, pantry, game assets, topdown lanes, hub visuals, evidence architecture, and archive. Recorded validator/script path-assumption risks, stale Level 1 / Button Goblin Clicker documentation candidates, and safe dry-run migration phases. No manifests or docs were moved, no validators were changed, no stale docs were corrected, no runtime/game code changed, and no PNG/source/derived/evidence/package/lock files were modified.

* **H5.86 Academy Visual Asset Pantry Human Review**: Recorded human/product review pass for the H5.85 Academy Visual Asset Pantry Index and Button Goblin Clicker background scene-anchor manifest. Accepted the pantry index as the Tier 1.5 visual integration planning source of truth, accepted 9 Button Goblin Clicker background anchors for scene-anchor planning only, preserved the doctrine that visual assets are Tier 1.5 and audio assets are future Tier 2.5, and confirmed runtime order starts with Button Goblin Clicker while Top-Down Slime Quest remains last-or-near-last because animation, tile behavior, placement, collision, pathfinding, and map rules remain deferred. No runtime wiring, game code, PNG edits, image processing, cleanup outputs, folder moves, manifest/docs reorganization, package/lock changes, or runtime approval occurred. H5.87 should plan manifest and asset docs organization before shelves get rearranged.
...
- Denied regions: farm-settlement.soil-plot-watered-sprout, farm-settlement.withered-crop-plot, farm-settlement.water-drop-token, farm-settlement.campfire, farm-settlement.smiling-sun, and farm-settlement.crescent-moon.
